...
1、先判断有没有环（快慢指针）
2、计算环中有几个节点n
3、双指针，第一个先走n步，然后一起走。相遇的地方就是环的入口


-----------------------------------------------------------
其实无需知道环中节点个数
第一步中慢指针走了k步，快指针走了2k步，此时k是环中节点数的整数倍。
这时可以让一个指针指向相遇时的节点，再让一个指针指向头节点，一起走，相遇时即为头结点。

public ListNode DetectCycle(ListNode head)
{
	//判断是否有环，返回相遇节点
	ListNode inLoop = grtNodeInLoop(head);
	if(inLoop == null){
		return null;
	}
	ListNode node = head;
...
```
方法一：
	遍历两个链表，分别加入两个栈，出栈，第一个不同的节点的next就是第一个重合节点。
	时间O(n+m)，空间O(m+n)
方法二：
	分别遍历一次，记下长度，双指针。
```
